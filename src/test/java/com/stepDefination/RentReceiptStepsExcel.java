...
public class RentReceiptStepsExcel {
    ...
    public void iNavigateToRentReceiptGeneratorPage(String browser) throws Exception {
        ...
        setup.navigateToUrl(ReadProperties.homePageURL());
...
//    @When("I fetch all data from sheet {string}")
//    public void i_fetch_all_data_from_sheet(String sheetName) {
//        try {
//            List<String[]> allData = excelReader.getAllRows(sheetName);
//            for (String[] data : allData) {
//                // Fetch data from each row
//                String tenantName = data[0];
//                String ownerName = data[1];
//                String rentAmount = data[2];
//                String rentalAddress = data[3];
//                String ownerAddress = data[4];
...
//
//                // Optionally click the generate button for each row
//                rentReceiptPage.clickGenerateReceiptButton("Generate Rent Receipt Now");
//            }
//        } catch (Exception e) {
//            e.printStackTrace();
//            throw new RuntimeException("Error processing multiple rows from the Excel sheet.", e);
//        }
//    }
//
//    
//}
    ...
    @When("I fetch data from sheet {string} row {string}")
    public void iFetchDataFromExcel(String sheetName, String rowIndexString) {
        // Retrieve data from the Excel file
    	int rowIndex = Integer.parseInt(rowIndexString);
    	
        String tenantName = excelReader.readData(sheetName, rowIndex, 0);
        String ownerName = excelReader.readData(sheetName, rowIndex, 1);
        String rentAmount = excelReader.readData(sheetName, rowIndex, 2);
        String rentalAddress = excelReader.readData(sheetName, rowIndex, 3);
        String ownerAddress = excelReader.readData(sheetName, rowIndex, 4);
...
//
    @And("I fetch email from sheet {string} row {string}")
    public void iEnterEmailAddress(String sheetName, String rowIndexString) {
    	int rowIndex = Integer.parseInt(rowIndexString);
    	
    	String emailAddress = excelReader.readData(sheetName, rowIndex, 5);
        rentReceiptPage.getEmailField().clear();
        rentReceiptPage.getEmailField().sendKeys(emailAddress);
    }
//
...
//    
    ...
    public void theOptionToDownloadPDFShouldBeAvailable() throws InterruptedException {
        ...
        rentReceiptPage.screenShot(driver, "C:\\Users\\PRIYESIN\\eclipse-workspace\\io.Testing-NoBroker\\ScreenShots\\Congralution.jpg");
